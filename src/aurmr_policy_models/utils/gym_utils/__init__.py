...
from aurmr_policy_models.envs.wrappers.multi_step import MultiStep

def make_env(cfg: DictConfig, multistep=False, num_obs_steps=1, num_action_steps=1):
    def _env_fn():
        env = hydra.utils.instantiate(cfg)
        if multistep:
            env = MultiStep(env, num_obs_steps, num_action_steps)
        return env
    return _env_fn


def make_async_vector_env(cfg, num_envs=4, multistep=False, num_obs_steps=1, num_action_steps=1):
    env_fns = [make_env(cfg, multistep, num_obs_steps, num_action_steps) for _ in range(num_envs)]
...
def reset_venv_all(venv, verbose=False, options_venv=None, **kwargs):
    if options_venv is None:
        options_venv = [
            {k: v for k, v in kwargs.items()} for _ in range(venv.n_envs)
        ]
    ...
    if isinstance(obs_venv, list):
        obs_venv = {
            key: np.stack([obs_venv[i][key] for i in range(venv.n_envs)])
            for key in obs_venv[0].keys()
        }
    if verbose:
        for index in range(venv.n_envs):
            logging.info(
...
def reset_venv(venv, env_ind, verbose=False):
    task = {}
    obs = venv.reset_one_arg(env_ind=env_ind, options=task)
    if verbose:
        logging.info(f"<-- Reset environment {env_ind} with task {task}")
    return obs


def set_venv_seeds(venv, seed):
    venv.seed(
        [seed + i for i in range(venv.n_envs)]
    )

def render_venv_all(venv):
    venv.call_async('render', mode='rgb_array')
    results = venv.call_wait()
    return results
